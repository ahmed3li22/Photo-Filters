    ...
    while(true)
        {
            ...
            cout<<"J) T.V effect on Image"<<endl;
            cout<<"K) Rotate Image "<<endl;
            cout<<"L) Put Frame on Image "<<endl;
            cout<<"M) Blur Image "<<endl;
            cout<<"N) sunny effect on Image"<<endl;
            cout<<"O) purple effect on Image "<<endl;
            cout<<"P) Save Image "<<endl;
            cout<<"Q) Load New Image "<<endl;
...
Image The_Land_Of_Wano(Image &image) {
    ...
    double blue_multiplier = 0.9;

    for (int i = 0; i < image.width; i++) {
        for (int j = 0; j < image.height; j++) {
            for (int k = 0; k < image.channels; k++) {
                if (k == 0) {
                    image(i , j , k) = min(static_cast<int>(image(i , j , k) * red_multiplier), 255);
                } else if (k == 1) {
                    image(i , j , k) = min(static_cast<int>(image(i , j , k) * green_multiplier), 255);
                } else {
                    image(i , j , k) = min(static_cast<int>(image(i , j , k) * blue_multiplier), 255);
                }
            }
        }
    }
...
Image noise(Image &image){
    ...
    int pixels2=(rand()%pixels)+1000;
    if(pixels2>pixels)
        {
            pixels2=pixels2-1000;
        }
    ...
    for(int i=0;i<pixels2;i++)
        {
            int x_coord=(rand()%image.width);
            int y_coord=(rand()%image.height);
            for(int j=0;j<3;j++)
                {
                    image(x_coord,y_coord,j)=((image(x_coord,y_coord,j)+(rand()%255))%255);
                }
        }
    return image;
}
